...
export default function Alarm() {
  ...
  const fetchData = async () => {
    try {
      ...
      const mentionResponse = await fetch(`/api/slackMembers`);
  ...
  const messages = [{ label: '사막전', value: '사막전' }];
  ...
  const scheduleSlackMessage = async (messageText: string, timestamp: number) => {
    const response = await fetch('/api/slackMessage', {
      ...
      body: JSON.stringify({
        ...
        text: `${selectedUsers} ${messageText}`,
        postAt: timestamp,
      }),
    });
              ...
              <div className={styles.radios}>
                {users.map((user, index) => (
                  <div className={styles.radio} key={index}>
                    <label htmlFor={`u${user.id}`}>{user.name}</label>
                    <input
                      type="checkbox"
                      value={user.id}
                      id={`u${user.id}`}
